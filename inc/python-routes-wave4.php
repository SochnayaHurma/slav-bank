...
{
    $wave4 = sb_python_wave4_partial_for_route($route_key);
    if ($wave4 !== '') {
        return $wave4;
    }
...
add_action('template_redirect', 'sb_python_wave4_template_redirect', 1);

function sb_python_wave4_template_redirect(): void
{
    ...
    if ($route_key !== '' && sb_python_wave4_has_local_override($route_key)) {
        return;
    }
...
add_filter('template_include', 'sb_python_wave4_template_include', 30);

function sb_python_wave4_template_include(string $template): string
{
    $route_key = sb_python_current_route_key();
    if ($route_key !== '' && sb_python_wave4_has_local_override($route_key)) {
        $candidate = get_theme_file_path('page-python-route-override-wave4.php');
